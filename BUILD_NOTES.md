...
| 5 | `docker-compose.repo.yml` mcp-bitbucket | **RESOLVED:** upstream `ghcr.io/ibrahimogod/bitbucket-mcp` is a **stdio-only** musl binary (`/app/bitbucket_mcp`) that crash-loops as a standalone sidecar. Added `mcp-bitbucket/Dockerfile` wrapping it with `supergateway` (like `mcp-github`); final stage is `node:20-alpine` because the binary is musl-linked. Overlay now `build`s `mcp-bitbucket-sse:local`. Verified: container stays up, SSE serves `200 text/event-stream` at `http://mcp-bitbucket:8080/sse`. Real `BITBUCKET_*` creds still needed for actual queries. |
